require 'rails_admin/adapters/mongomapper/abstract_object'
require 'rails_admin/adapters/mongomapper/property'
require 'rails_admin/adapters/mongomapper/associations'

# TODO go over all Mongoid code, replicate all I can
module RailsAdmin
  module Adapters
    module MongoMapper
      def new(params = {})
      ...
      def get(id)
        model.where(primary_key => id).first
      end
      ...
      def all(options = {}, scope = nil)
        ...
        scope = scope.find(_id: options[:bulk_ids]) if options[:bulk_ids]
      ...
      def embedded?
        model.ancestors.include?(::MongoMapper::EmbeddedDocument)
      end

      def properties
        model.keys.collect { |_name, field| Property.new(field, model) }
      end
      ...
      def associations
        model.associations.values.map do |a|
          Association.new(a, model)
        end
      end
      ...
      private
        ...
        def parse_collection_name(column)
          ...
          if associations.detect { |a| a.name == collection_name.to_sym }.try(:embeddable?)
            [table_name, column]
          else
        ...
        def sort_by(options, scope)
          ...
          if options[:sort_reverse]
            ...
            scope.sort field_name.to_sym.desc
          end
        end
